...
// ── K‑Shield SVG Logo ──────────────────────────────────────────────────
const KSHIELD_SVG = `<svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M28 4 L46 12 L46 28 C46 38 36 48 28 52 C20 48 10 38 10 28 L10 12 Z" fill="none" stroke="#C8963E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M20 20 L20 36 M20 28 L32 20 M20 28 L32 36" stroke="#C8963E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
const KSHIELD_SVG_SMALL = `<svg width="40" height="40" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M28 4 L46 12 L46 28 C46 38 36 48 28 52 C20 48 10 38 10 28 L10 12 Z" fill="none" stroke="#C8963E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M20 20 L20 36 M20 28 L32 20 M20 28 L32 36" stroke="#C8963E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
const KSHIELD_SVG_ONBOARDING = `<svg width="72" height="72" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M28 4 L46 12 L46 28 C46 38 36 48 28 52 C20 48 10 38 10 28 L10 12 Z" fill="rgba(200,150,62,0.1)" stroke="#C8963E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M20 20 L20 36 M20 28 L32 20 M20 28 L32 36" stroke="#C8963E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;

(function() {
  const style = document.createElement('style');
  style.textContent = `
    #kappa-badge{position:fixed!important;bottom:24px!important;right:24px!important;width:56px!important;height:56px!important;border-radius:50%!important;border:3px solid rgba(255,255,255,1)!important;display:flex!important;align-items:center!important;justify-content:center!important;cursor:pointer!important;z-index:99999!important;box-sizing:border-box!important;margin:0!important;padding:0!important;backdrop-filter:blur(8px)!important;-webkit-backdrop-filter:blur(8px)!important}
    ...
    #kappa-card{position:fixed!important;bottom:88px!important;right:24px!important;width:320px!important;max-height:440px!important;background:rgba(253,246,236,0.94)!important;backdrop-filter:blur(24px)!important;-webkit-backdrop-filter:blur(24px)!important;border:0.5px solid rgba(200,150,62,0.2)!important;border-radius:18px!important;box-shadow:0 8px 32px rgba(44,36,22,0.08),0 2px 8px rgba(44,36,22,0.04),inset 0 1px 0 rgba(255,255,255,0.5)!important;padding:18px!important;z-index:99998!important;overflow-y:auto!important;font-family:system-ui,sans-serif!important;color:#2C2416!important;animation:card-up 0.3s ease!important;box-sizing:border-box!important;margin:0!important;line-height:1.5!important}
...
let lastConversationHash = '', lastMessageCount = 0, conversationScores = [], isAuditing = true, currentMode = 'balanced', cardDismissedForThisTurn = false;

chrome.storage.local.get(['isAuditing','kappa_user_mode','conversationScores'],(r)=>{if(r.isAuditing!==undefined)isAuditing=r.isAuditing;if(r.kappa_user_mode)currentMode=r.kappa_user_mode;if(r.conversationScores)conversationScores=r.conversationScores});

function detectModel(){const h=location.hostname;if(h.includes('chatgpt.com'))return'chatgpt';if(h.includes('gemini.google.com'))return'gemini';if(h.includes('claude.ai'))return'claude';return'unknown'}
function extractConversation(){const m=detectModel(),msgs=[];try{if(m==='chatgpt'){document.querySelectorAll('[data-message-author-role]').forEach(t=>{const r=t.getAttribute('data-message-author-role'),x=t.innerText.trim();if(x)msgs.push({role:r==='user'?'user':'assistant',content:x})})}else if(m==='gemini'){document.querySelectorAll('.user-query-text,.user-query-bubble,.model-response-text,.response-content,message-content').forEach(e=>{const x=e.innerText.trim();if(x){const u=e.classList.contains('user-query-text')||e.classList.contains('user-query-bubble');msgs.push({role:u?'user':'assistant',content:x})}})}else if(m==='claude'){document.querySelectorAll('[data-testid="user-message"],[data-testid="assistant-message"]').forEach(e=>{const x=e.innerText.trim();if(x){const t=e.getAttribute('data-testid');msgs.push({role:t.includes('user')?'user':'assistant',content:x})}})}}catch(e){console.error('Kappa DOM:',e.message)}return msgs}
function hashConversation(m){const s=m.map(x=>x.role+x.content.slice(0,100)).join('|');let h=0;for(let i=0;i<s.length;i++){h=((h<<5)-h)+s.charCodeAt(i);h|=0}return h.toString()}
function pauseKey(){return'kappa_audit_paused_'+location.hostname}
function toggleAuditing(){isAuditing=!isAuditing;chrome.storage.local.set({[pauseKey()]:!isAuditing});if(isAuditing){updateBadge('green')}else{injectPausedBadge();const c=document.getElementById('kappa-card');if(c)c.remove()}}
function detectLanguage(t){const x=t.toLowerCase();if(/[\u0600-\u06FF]/.test(t))return'ar';if(/\b(na|kwa|ya|za|wa|ni|kwenye|katika|hii|hizi|hiyo|hayo|hili|haya)\b/.test(x))return'sw';if(/\b(je|tu|il|elle|nous|vous|ils|elles|le|la|les|un|une|des|est|sont|dans|pour|avec|sur|ce|cette|ces|mon|ton|son|ma|ta|sa|mes|tes|ses|recherche|financement|chercheur|maroc|afrique)\b/.test(x))return'fr';return'en'}
function detectBrowserLanguage(){const l=navigator.language||'en';if(l.startsWith('fr'))return'fr';if(l.startsWith('ar'))return'ar';if(l.startsWith('sw'))return'sw';return'en'}

const whispers={en:["Small steps every day.","Nothing is impossible.","You are allowed to take your time.","Your curiosity is a strength.","Keep going. You're building something.","You belong in every room you enter.","Progress, not perfection.","Your voice matters.","One idea can change everything.","The world needs what you're building."],fr:["Des petits pas chaque jour.","Rien n'est impossible.","Tu as le droit de prendre ton temps.","Ta curiosité est une force.","Continue. Tu construis quelque chose.","Tu as ta place dans chaque pièce où tu entres.","Le progrès, pas la perfection.","Ta voix compte.","Une seule idée peut tout changer.","Le monde a besoin de ce que tu construis."],ar:["خطوات صغيرة كل يوم.","لا شيء مستحيل.","يسمح لك بأخذ وقتك.","فضولك هو قوة.","استمر. أنت تبني شيئاً.","أنت تنتمي إلى كل غرفة تدخلها.","التقدم، ليس الكمال.","صوتك مهم.","فكرة واحدة يمكن أن تغير كل شيء.","العالم يحتاج إلى ما تبنيه."],sw:["Hatua ndogo kila siku.","Hakuna lisilowezekana.","Unaruhusiwa kuchukua muda wako.","Udadisi wako ni nguvu.","Endelea. Unajenga kitu.","Una nafasi katika kila chumba unachoingia.","Maendeleo, si ukamilifu.","Sauti yako ina maana.","Wazo moja linaweza kubadilisha kila kitu.","Dunia inahitaji unachojenga."]};
...
function showLearnMore(topic, lang) {
  ...
  overlay.innerHTML = `<div style="background:rgba(253,246,236,0.95);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);border:0.5px solid rgba(200,150,62,0.25);border-radius:18px;padding:24px;max-width:400px;max-height:70vh;overflow-y:auto;box-shadow:0 16px 48px rgba(44,36,22,0.08);line-height:1.6"><h3 style="color:#D4734B;margin-bottom:10px">${title}</h3><p style="font-size:13px;color:#2C2416;margin-bottom:8px">${body}</p><button class="kappa-btn" id="kappa-learn-close">${t.closeLearn || 'Close'}</button></div>`;
  document.body.appendChild(overlay);
  document.getElementById('kappa-learn-close').addEventListener('click', () => overlay.remove());
...
function injectBadge(verdict){const old=document.getElementById('kappa-badge');if(old)old.remove();const map={green:'green',orange:'amber',red:'red'};const cls=map[verdict]||'green';const b=document.createElement('div');b.id='kappa-badge';b.className=`aura-${cls}`;b.innerHTML=KSHIELD_SVG_SMALL;b.title='Kappa — click for details';document.body.appendChild(b);b.addEventListener('click',()=>{if(!isAuditing){toggleAuditing();return}chrome.storage.local.get(['lastAudit','lastAuditLang'],(d)=>{if(d.lastAudit){showCard(d.lastAudit,d.lastAuditLang||'en')}else{const t=i18n[detectBrowserLanguage()]||i18n.en;const tip=document.createElement('div');tip.className='kappa-tooltip';tip.innerHTML=`<span style="font-size:12px">⏳ ${t.waiting}</span>`;document.body.appendChild(tip);setTimeout(()=>{if(tip.parentNode)tip.remove()},2000)}})})}
function injectPausedBadge(){const old=document.getElementById('kappa-badge');if(old)old.remove();const b=document.createElement('div');b.id='kappa-badge';b.className='aura-paused';b.innerHTML='▶️';b.title='Kappa — paused, click to resume';document.body.appendChild(b);b.addEventListener('click',()=>{toggleAuditing()})}
function updateBadge(verdict){const b=document.getElementById('kappa-badge');if(!b||!isAuditing)return;const map={green:'green',orange:'amber',red:'red'};b.className=`aura-${map[verdict]||'green'}`;b.innerHTML=KSHIELD_SVG_SMALL}

async function auditConversation(messages){if(!isAuditing)return;const model=detectModel();const hash=hashConversation(messages);if(hash===lastConversationHash&&messages.length<=lastMessageCount)return;if(messages.length<2)return;const last=messages[messages.length-1];if(last.role!=='assistant')return;lastConversationHash=hash;lastMessageCount=messages.length;try{const resp=await fetch(BACKEND_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conversation:messages,model_name:model,user_location:'Africa'})});if(!resp.ok)return;const audit=await resp.json();conversationScores.push(audit.current_cdi);if(conversationScores.length>10)conversationScores.shift();const lang=detectLanguage(last.content);chrome.storage.local.set({lastAudit:audit,lastAuditLang:lang,conversationScores:conversationScores});updateBadge(audit.verdict);if(cardDismissedForThisTurn)return}catch(err){console.log('Kappa backend not reachable:',err.message)}}

function startObserver(){let timer,prevCount=0;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{const msgs=extractConversation();const cur=msgs.length;if(cur<prevCount&&cur<=1){lastConversationHash='';lastMessageCount=0;conversationScores=[];const c=document.getElementById('kappa-card');if(c)c.remove();chrome.storage.local.remove(['lastAudit','lastAuditLang','conversationScores']);injectBadge('green');prevCount=cur;return}if(cur>prevCount&&msgs.length>=2){const last=msgs[msgs.length-1];if(last.role==='user'){cardDismissedForThisTurn=false;const c=document.getElementById('kappa-card');if(c)c.remove();chrome.storage.local.remove(['lastAudit','lastAuditLang']);const lang=detectLanguage(last.content);const t=i18n[lang]||i18n.en;const tip=document.createElement('div');tip.className='kappa-tooltip';tip.innerHTML=`<span style="font-size:12px">⏳ ${t.waiting}</span>`;document.body.appendChild(tip);setTimeout(()=>{if(tip.parentNode)tip.remove()},2500)}}prevCount=cur;if(msgs.length>=2)auditConversation(msgs)},800)}).observe(document.body,{childList:true,subtree:true})}

function checkOnboarding(){chrome.storage.local.get(['kappa_user_mode'],(r)=>{chrome.storage.local.remove(['lastAudit','lastAuditLang']);if(!r.kappa_user_mode){const lang=detectBrowserLanguage();const t=i18n[lang]||i18n.en;const o=document.createElement('div');o.id='kappa-onboarding';o.innerHTML=`<div style="position:fixed;inset:0;background:rgba(20,16,10,0.5);backdrop-filter:blur(12px);display:flex;align-items:center;justify-content:center;z-index:100000"><div style="background:rgba(253,246,236,0.94);backdrop-filter:blur(24px);border:0.5px solid rgba(200,150,62,0.2);border-radius:18px;padding:32px 24px;max-width:380px;text-align:center;box-shadow:0 16px 48px rgba(44,36,22,0.08)">${KSHIELD_SVG_ONBOARDING}<h2 style="font-size:20px;color:#D4734B;margin-bottom:8px">${t.welcomeTitle}</h2><p style="font-size:13px;color:#8C8C7C;margin-bottom:18px">${t.welcomeText}</p><p style="font-size:11px;opacity:0.7">🔒 ${t.privacy}</p><p style="margin-top:18px;font-weight:500">${t.chooseMode}</p><div style="display:flex;gap:10px;justify-content:center;margin:16px 0 20px"><div class="km-card selected" data-mode="simple" style="background:rgba(255,255,255,0.5);border:1px solid transparent;border-radius:12px;padding:12px 8px;width:90px;cursor:pointer"><div style="font-size:24px">🟢</div><div style="font-weight:600;font-size:13px;margin:6px 0 2px">${t.simple}</div><div style="font-size:10px;color:#8C8C7C">${t.simpleDesc}</div></div><div class="km-card" data-mode="balanced" style="background:rgba(255,255,255,0.5);border:1px solid transparent;border-radius:12px;padding:12px 8px;width:90px;cursor:pointer"><div style="font-size:24px">🟡</div><div style="font-weight:600;font-size:13px;margin:6px 0 2px">${t.balanced}</div><div style="font-size:10px;color:#8C8C7C">${t.balancedDesc}</div></div><div class="km-card" data-mode="expert" style="background:rgba(255,255,255,0.5);border:1px solid transparent;border-radius:12px;padding:12px 8px;width:90px;cursor:pointer"><div style="font-size:24px">🔴</div><div style="font-weight:600;font-size:13px;margin:6px 0 2px">${t.expert}</div><div style="font-size:10px;color:#8C8C7C">${t.expertDesc}</div></div></div><button class="kappa-btn" id="kappa-start">${t.getStarted}</button></div></div>`;document.body.appendChild(o);let sel='simple';o.querySelectorAll('.km-card').forEach(c=>{c.addEventListener('click',()=>{o.querySelectorAll('.km-card').forEach(x=>x.style.borderColor='transparent');c.style.borderColor='#C8963E';c.style.boxShadow='0 0 12px rgba(200,150,62,0.3)';sel=c.dataset.mode})});document.getElementById('kappa-start').addEventListener('click',()=>{currentMode=sel;chrome.storage.local.set({kappa_user_mode:sel});o.remove();injectBadge('green')})}else{currentMode=r.kappa_user_mode;injectBadge('green')}})}
